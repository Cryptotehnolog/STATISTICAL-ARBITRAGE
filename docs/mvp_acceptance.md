...
Live-scale проверка Bybit/Binance/OKX/Deribit остается отдельным opt-in readiness run. Для
стартового Bybit smoke используется:

```powershell
.\scripts\check_live_market_data_acceptance.ps1
```

Команда нужна перед реальной research сессией или user acceptance testing, но не должна
ломать pre-commit и CI.
