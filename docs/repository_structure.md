...
- `.kiro/specs/` содержит planning specifications и breakdown задач. Это исходные
  проектные артефакты.
- `data/` содержит runtime data, локальные databases, ApeRAG/legacy LightRAG stores,
  vector indexes и
...
- `docs/knowledge_graph/` содержит legacy generated HTML viewer и JSON export для LightRAG. Эта
  директория ignored.
...
- `src/stat_arb/memory/` содержит active ApeRAG client boundary и legacy LightRAG code,
  который будет удален после переключения Memory Agent и human graph inspection.
...
- `scripts/check_memory_backend.ps1` проверяет active memory backend. Сейчас это ApeRAG,
  без вызовов legacy LightRAG checks.
...
- `scripts/seed_lightrag_curated.ps1` делает preview/apply только для
  `docs/knowledge/*.md` shards. Decisions memory разделена на thematic shards, поэтому
  новые решения нужно добавлять в самый маленький релевантный файл, а не в общий index.
  Для уже существующей persistent базы предпочтительнее
  `scripts/rebuild_lightrag_curated.ps1`, пока нет безопасной замены source docs без
  duplicate records.
...
- `scripts/rebuild_lightrag_curated.ps1` делает backup текущей persistent LightRAG runtime
  storage, заново сидит `docs/knowledge/*.md` и запускает memory guard.
...
- `scripts/check_lightrag_memory_fresh.ps1` проверяет, что curated LightRAG memory
  синхронизирована с `docs/knowledge/*.md`, persistent doc_status здоровый, graph export
  проходит, human-facing viewer export обновлен, а контрольный query отвечает по проектной
  памяти.
