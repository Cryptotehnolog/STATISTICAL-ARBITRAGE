...
- `src/stat_arb/memory/` содержит active ApeRAG client boundary и Memory Agent policy
  layer.
...
- `scripts/stop_runtime_infra.ps1` мягко останавливает тяжелую локальную runtime
  инфраструктуру проекта: ApeRAG, Infisical, OmniRoute и embedding server. Скрипт не
  удаляет Docker volumes и показывает память `VmmemWSL`.
- `scripts/start_runtime_infra.ps1` поднимает эту runtime инфраструктуру обратно и может
  выполнить быстрые readiness checks.
