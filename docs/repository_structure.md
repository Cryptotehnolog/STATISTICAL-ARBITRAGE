...
- `src/stat_arb/agents/` содержит agent service boundaries: hypothesis, statistical
  testing, backtest, critic, report, coordinator и failure handling.
- `src/stat_arb/backtest/` содержит backtesting core, costs, metrics, baseline comparison,
  sensitivity, walk-forward и reproducibility logic.
- `src/stat_arb/cli/` содержит CLI command wiring, typed stage payload parsing и stage
  execution support.
- `src/stat_arb/dashboard/` содержит read-only Streamlit dashboard, registry projections
  и presentation helpers.
...
- `src/stat_arb/memory/` содержит active ApeRAG client boundary, `MemoryBackend`
  adapter contract, Memory Agent query/write service, policy filters и degraded write
  queue contracts.
...
- `src/stat_arb/reports/` содержит deterministic report generation.
- `src/stat_arb/statistical/` содержит statistical testing logic: cointegration,
  stationarity, hedge ratio, z-score, regime и residual diagnostics.
...
- `scripts/check_memory_health.ps1` объединяет проверку active project memory backend и
  отдельной operational agent memory collection.
- `scripts/check_memory_agent_pipeline.ps1` запускает deterministic Memory Agent contract
  tests и boundary guards; runtime ApeRAG smoke включается только через
...
- `scripts/clean_runtime_artifacts.ps1` показывает и удаляет regenerable runtime/cache
  ...
  volumes.
- `docs/runtime_maintenance.md` описывает, когда безопасно запускать ручную очистку,
  почему сначала нужен dry-run, и какие persistent данные нельзя удалять обычным cleanup.
...
- `scripts/check_no_legacy_memory_backend_user_surface.ps1` не дает вернуть legacy memory
