...
Equities ingestion не включается как raw OHLCV workflow. Любой `Yahoo`, `Alpaca` или
`Polygon` dataset должен иметь `adjustment_mode=split_dividend`, то есть split и dividend
adjustments уже применены и отражены в provenance. `adjustment_mode=none`, `split` или
`dividend` по отдельности для equities не считаются достаточными для research/backtest
boundary.

Пока такого adjusted-data слоя нет, equities остаются roadmap-направлением, а не активным
MVP data source.
